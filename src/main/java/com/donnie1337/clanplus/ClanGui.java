...
public final class ClanGui {
    ...
    private double kdr(UUID uuid) { return plugin.clans().kdr(uuid); }
    private String coins(Player player) {
        String placeholder = plugin.getConfig().getString("coins.placeholder", "");
        if (placeholder == null || placeholder.isBlank()) return "N/D";
        if (!Bukkit.getPluginManager().isPluginEnabled("PlaceholderAPI")) return "N/D";
        try {
            Class<?> clazz = Class.forName("me.clip.placeholderapi.PlaceholderAPI");
            Method method = clazz.getMethod("setPlaceholders", org.bukkit.OfflinePlayer.class, String.class);
    ...
    public void openMain(Player p) {
        ...
        skull.setLore(List.of(
                ...
                color("&7Kills: &a" + kills),
                color("&7Mortes: &c" + deaths)
        ));
        profile.setItemMeta(skull);
        inv.setItem(4, profile);

        inv.setItem(10, item(c == null ? Material.EMERALD : Material.CHEST,
                c == null ? "&aCriar Clan" : "&bMeu Clan",
                c == null ? "&7Crie seu próprio clan." : "&7Gerencie seu clan."));
        ...
        inv.setItem(26, item(Material.BARRIER, "&cFechar"));
        p.openInventory(inv);
    }
    ...
    public void openInvites(Player p) {
        ...
        for (Clan c : plugin.clans().all()) {
            for (Clan.Invite invite : c.invites().values()) {
                if (!invite.player().equals(p.getUniqueId()) || slot >= 45) continue;
                ItemStack item = item(Material.PAPER, "&eConvite: &f" + c.name(),
                        "&7Tag: &6" + c.tag(),
                        "&7Líder: &f" + name(c.owner()),
                        "&7ID: &e" + invite.id(),
                        "",
        ...
        if (slot == 0) inv.setItem(22, item(Material.BARRIER, "&cNenhum convite", "&7Você não possui convites pendentes."));
    ...
    public void openTop(Player p) {
        ...
        for (int i = 0; i < Math.min(45, clans.size()); i++) {
            Clan c = clans.get(i);
            double kdr = plugin.clans().clanKdr(c);
            inv.setItem(i, item(i == 0 ? Material.GOLD_BLOCK : i == 1 ? Material.IRON_BLOCK : i == 2 ? Material.COPPER_BLOCK : Material.NAME_TAG,
                    "&e#" + (i + 1) + " &f" + c.name() + " &8[&6" + c.tag() + "&8]",
                    "&7KDR médio: &e" + String.format(Locale.US, "%.2f", kdr),
                    "&7Membros: &f" + c.members().size()));
        }
        inv.setItem(49, item(Material.ARROW, "&7Voltar"));
        p.openInventory(inv);
    }
    ...
    public void openAll(Player p) {
        ...
        for (Clan c : plugin.clans().all()) {
            if (slot >= 45) break;
            inv.setItem(slot++, item(Material.NAME_TAG, "&f" + c.name() + " &8[&6" + c.tag() + "&8]",
                    "&7Líder: &f" + name(c.owner()),
                    "&7Membros: &f" + c.members().size(),
                    "&7Online: &f" + c.onlineCount(),
                    "&7KDR médio: &e" + String.format(Locale.US, "%.2f", plugin.clans().clanKdr(c))));
        }
        if (slot == 0) inv.setItem(22, item(Material.BARRIER, "&cNenhum clan", "&7Ainda não existem clans."));
        inv.setItem(49, item(Material.ARROW, "&7Voltar"));
        p.openInventory(inv);
    }
    ...
    public void openKdr(Player p) {
        ...
        for (int i = 0; i < Math.min(45, players.size()); i++) {
            ...
            inv.setItem(i, item(Material.PLAYER_HEAD, "&e#" + (i + 1) + " &f" + String.valueOf(target.getName()),
                    "&7Clan: &f" + (c == null ? "Nenhum" : c.name() + " [" + c.tag() + "]"),
                    "&7KDR: &e" + String.format(Locale.US, "%.2f", plugin.clans().kdr(uuid)),
                    "&7Kills: &a" + plugin.clans().kills(uuid),
                    "&7Mortes: &c" + plugin.clans().deaths(uuid)));
        }
        inv.setItem(49, item(Material.ARROW, "&7Voltar"));
        p.openInventory(inv);
    }
    ...
    private String role(ClanRole role) {
        if (role == null) return "Nenhum";
        return switch (role) { case LEADER -> "Líder"; case MODERATOR -> "Moderador"; case MEMBER -> "Membro"; };
    }
}
