...
public final class ClanPlus extends JavaPlugin {
    ...
    private final Map<String,Inventory> openClanChests = new HashMap<>();

    @Override public void onEnable(){
        saveDefaultConfig(); saveResource("messages.yml",false); loadMessages();
        clanManager=new ClanManager(this); clanManager.load();
        ClanCommand clanCommand=new ClanCommand(this); PluginCommand clan=getCommand("clan"); if(clan!=null){clan.setExecutor(clanCommand);clan.setTabCompleter(clanCommand);}
        PluginCommand clans=getCommand("clans"); if(clans!=null)clans.setExecutor(new ClansCommand(this));
        PluginCommand admin=getCommand("clanadmin"); if(admin!=null){ClanAdminCommand a=new ClanAdminCommand(this);admin.setExecutor(a);admin.setTabCompleter(a);}
        getServer().getPluginManager().registerEvents(new ClanListener(this),this); getServer().getPluginManager().registerEvents(new ClanFeatureListener(this),this);
        long interval=6000L;getServer().getScheduler().runTaskTimer(this,()->{if(clanManager!=null){saveOpenChests();clanManager.save();}},interval,interval);
    ...
    public void persistClanChest(Clan clan,Inventory inventory){if(clan!=null&&inventory!=null&&isManagedClanChest(inventory)){clan.setChest(inventory.getContents());clanManager.save();}}
    ...
    public String getPlayerTag(UUID playerId){if(playerId==null||clanManager==null)return"";Clan clan=clanManager.byPlayer(playerId);return clan==null?"":clan.tag();}
